...
public class CalcServer3 {
  ...
  static class OtherExcution extends Thread {
    Socket socket;

    OtherExcution(Socket socket) {
      this.socket = socket;
    }
  ...
  public static void main(String[] args) throws Exception {
    ...
    while (true) {
      ...
      OtherExcution 클라이언트요청처리 = new OtherExcution(socket);
      클라이언트요청처리.start();
      // main() 메서드의 실행과 분리해 별도고 run() 메서드를 호출한 후 즉시 리턴한다.
    }
    // ss.close();
  }
}
