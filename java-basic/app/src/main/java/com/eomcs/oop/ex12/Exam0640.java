...
public class Exam0640 {
  ...
  public static void main(String[] args) {
    ...
    // 1) My의 m()는 스태틱 메서드 아님
    // 2) My의 m()는 String 파라미터 못받음

    // My obj = new My();
    // Predicate<String> p2 = obj::m; // 컴파일 오류
    // 1) My의 m()는 String 파라미터를 못 받음
    // 1) 기존의 My의 m()을 test 메서드를 구현하는데에 사용하기
    Predicate<My> obj = (My value) -> {
      return value.m();
    };
    // 2) 코드를 좀 더 줄이기
    Predicate<My> obj2 = value -> value.m();
    // 3) 코드를 더 줄이기 - 바로 위의 코드를 작성하는 경우가 많다 보니 다음과 같은 단축 문법 등장
