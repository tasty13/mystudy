...
public class ServerApp {
  ...
  void run() {
    ...
    try {
      // 1) 랜카드 연결 정보를 준비한다.
      // -> 랜카드와 연결하는 것은 실제 OS가 수행
      // -> JVM은 OS가 작업한 결과를 가져온다.
      // new ServerSocket(포트번호)
      // -> 포트번호: 외부에서 랜카드로 들어온 데이터를 받을 떄 사용할 식별 번호 (중복 불가)
      ServerSocket serverSocket = new ServerSocket(8888);
      System.out.println("서버 실행!");

      // 2) 클라이언트의 연결을 기다림
      // -> 클라이언트 대기 목록에서 먼저 연결된 순서대로 클라이언트 연결 정보를 꺼낸다.
      // -> 클라이언트 대기 목록에 아무것도 없다면 연결이 될 떄까지 리턴하지 않고 대기
      ...
      while (true) {
        ...
        Method[] methods = dao.getClass().getDeclaredMethods(); // 현재 클래스 객체에 선언된 메서드 뽑아냄
        Method commandHandler = null;
        for (Method m : methods) {
          if (m.getName().equals(command)) {
            commandHandler = m; // 일치하는 객체를 저장
            break;
          }
        }
        ...
        // -> 1개만 있다는 가정 하에 아규먼트 값 준비
        if (params.length > 0) {
          // 파라미터 타입을 알아낸다.
          Class<?> paramType = params[0].getType();

          // 클라이언트가 보낸 JSON 문자열을 해당 파라미터 타입 객체로 변환
