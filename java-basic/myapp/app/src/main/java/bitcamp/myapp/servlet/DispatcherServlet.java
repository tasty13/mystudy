...
public class DispatcherServlet extends HttpServlet {
  ...
  private Object createValueObject(Class<?> type, HttpServletRequest request) throws Exception {
    ...
    // 2) 생성자를 이용해 도메인 객체 생성
    Object obj = constructor.newInstance();

    // 3) 도메인 클래스의 메서드 목록 가져옴
    Method[] methods = type.getDeclaredMethods();

    // 4) 메서드 중에서 setter 메서드 알아냄
    for (Method setter : methods) {
      if (!setter.getName().startsWith("set")) {
        continue;
      }

      // 5) setter 메서드 이름에서 프로퍼티 이름 추출
      // 예) setFirstName => firstName
      String propName =
          Character.toUpperCase(setter.getName().charAt(3)) + setter.getName().substring(4);

      // 6) 프로퍼티 이름으로 넘어온 요청 파라미터 값을 꺼낸다.
      String requestParamValue = request.getParameter(propName);

      // 7) 도메인 객체의 프로퍼티 이름과 일치하는 요청 파라미터 값이 있다면 객체에 저장
      if (requestParamValue != null) {
        // setter 메서드의 파라미터 타입 알아냄
